...
class Embedder:
    ...
    async def make_embed(self, ctx, em_name=None, time=False):
        ...
        if em_name:
            ...
            if time:
                ...
                footer = 'embed created on {} UTC by {}'.format(timestamp,
                                                                author.name)
            else:
            ...
            if em_name and title and content and footer:
                ...
                self.save_json()
                await self.bot.say("Embed named \"{}\" stored."
                                   .format(em_name))
        else:
    ...
    async def rm_embed(self, ctx, em_name=None):
        ...
        if em_name:
            if server.id in self.settings:
                ...
                self.save_json()
                if key:
                    await self.bot.say("Embed stored by name: \"{}\" "
                                       "has been removed".format(key))
                else:
                    await self.bot.say("No such embed.")
            else:
                await self.bot.say("I don't have any embeds stored "
                                   "for this server")
        else:
            await self.bot.say("I was expecting the name of "
                               "the embed to remove")
    ...
    async def get(self, ctx, em_name=None, pm=False):
        ...
        if em_name:
            title = self.settings.get(server.id, {}) \
                .get(em_name, {}).get('title')
            content = self.settings.get(server.id, {}) \
                .get(em_name, {}).get('content')
            footer = self.settings.get(server.id, {}) \
                .get(em_name, {}).get('footer')
            if content and footer and title:
                ...
                await self.bot.say("I couldn't find an "
                                   "embed by that name for this server.")

        else:
            await self.bot.say("I was expecting the "
                               "name of the embed you wanted.")
    ...
    async def embed_list(self, ctx):
        ...
        if ems:
            ...
            await self.bot.say("I don't have any embeds "
