...
class AdvRoleAssign:
    ...
    async def setremovable(self, ctx, *roles: discord.Role):
        ...
        output = ""
        for role in self.advroleset_filter(user, roles, yld=True):
    ...
    async def advset_mutualexclusive(self, ctx, *roles: discord.Role):
        ...
        if len(self.advroleset_filter(roles)) != len(unique(roles)):
    ...
    async def listroles(self, ctx):
        ...
        output = ""
        for role in self.get_joinable(author, yld=True):
    ...
    def get_conflicting(self, who: discord.Member,
                        role: discord.Role, **kwargs):
        server = who.server
