...
class ChannelRedirect(commands.Cog):
    ...
    def __init__(self, bot, *args, **kwargs):
        ...
        self.config = Config.get_conf(
            self, identifier=78631113035100160 + 1, force_registration=True
        )
    ...
    def should_early_exit(conf: dict, com: commands.Command):
        ...
        with contextlib.suppress(KeyError):
            if conf["com_allowlist"]["command"][com.qualified_name]:
                return True
        with contextlib.suppress(KeyError):
            if conf["com_allowlist"]["cog"][com.cog_name]:
                return True
    ...
    async def get_allowed_channels(
        ...
        *,
        ...
        if gset["mode"] == "allowlist":
            allowed_ids = {int(idx) for idx in gset["allowlist"]}

        elif gset["mode"] == "blocklist":
            disallowed_ids = {int(idx) for idx in gset["blocklist"]}
        ...
        return {channel for channel in channels if channel.id in allowed_ids}
    ...
    async def rset_allowlistcom_add(
        ...
        """
        await self.config.guild(ctx.guild).com_allowlist.set_raw(
            *cog_or_command, value=True
        )
        await ctx.tick()

    @rset_except.command(name="unallowcommandeverywhere")
    async def rset_allowlistcom_rem(
        self, ctx: commands.Context, *, cog_or_command: CogOrCOmmand
    ):
        """
