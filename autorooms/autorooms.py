...
class AutoRooms:
    ...
    async def autorooms(self, memb_before, memb_after):
        ...
        a_server = memb_after.server

        if a_server.id in self.settings:
            a_channels = self.settings[a_server.id]['channels']
            a_cache = self.settings[a_server.id]['cache']
            a_clones = self.settings[a_server.id]['clones']

            if self.settings[a_server.id]['toggleactive']:
                if memb_after.voice.voice_channel is not None:
                    chan = memb_after.voice.voice_channel
                    if chan.id in a_channels:
                        ...
                        if self.settings[a_server.id].get('toggleowner',
                                                          False):
                            ...
                            await self.bot.edit_channel_permissions(channel,
                                                      memb_after, overwrite)
                        self.settings[a_server.id]['clones'].append(channel.id)
                    self.save_json()

            if memb_after.voice.voice_channel is not None:
                channel = memb_after.voice.voice_channel
                if channel.id in a_clones:
                    if channel.id not in a_cache:
                        a_cache.append(channel.id)
                        self.save_json()

        if b_server.id in self.settings:
            b_channels = self.settings[b_server.id]['channels']
